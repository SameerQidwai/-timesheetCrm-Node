...
export class ReportExportBackupController {
  ...
  async benchResources(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      for (let resource of resources) {
        let resourceSkills: Array<any> = [];

        exportData.push(
          Object.keys(resource).map((key: any) => {
            let getValue = (resource as any)[key];
            if (key === 'skills') {
              getValue.forEach((skill: any) =>
                resourceSkills.push([
                  '',
                  '',
                  '',
        ...
        exportData = [...exportData, ...resourceSkills];
      }
  ...
  async workforceSkills(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      for (let workforce of worforce) {
        exportData.push(this._flatMap(workforce));
      }
  ...
  async exportReport(reportType: string, data: any): Promise<Boolean> {
    fs.writeFileSync(
      path.join(__dirname, `../../public/reports/report.xlsx`),
      ''
    );
    ...
    // Writing to our file
    xlsx.writeFile(workbook, `./public/reports/report.xlsx`);

    return true;
  }

  _flatMap(data: any): Array<any> {
    let response: Array<any> = [];

    Object.keys(data).map((key: any) => {
      response.push((data as any)[key]);
    });
