...
export class SubContractorRepository extends Repository<Employee> {
    ...
    async createAndSave(subContractor: SubContractorDTO): Promise<any> {
        ...
        id = await this.manager.transaction(async transactionalEntityManager => {            
            ...
            let organizationObj = await transactionalEntityManager.findOne(Organization, subContractor.organizationId);
            if(!organizationObj) {
                throw Error("Must provide organization");
            }
            ...
            let contactPersonObj = await transactionalEntityManager.findOne(ContactPerson, subContractor.contactPersonId, { relations: ["contactPersonOrganizations"]});
            if(!contactPersonObj) {
                throw Error("Must provide contact person");
            }
            
            // find contactpersonorganization id for oneLM
            let contactPersonOrganization = contactPersonObj.contactPersonOrganizations.filter(x => x.organizationId == subContractor.organizationId)[0];
            ...
            if (subContractor.stateId) {
                state = await transactionalEntityManager.findOne(State, subContractor.stateId);
                if (!state) {
    ...
    async getAllActive(): Promise<any[]> {
        let result = await this.find({
            relations: ["contactPersonOrganization", "contactPersonOrganization.contactPerson", "contactPersonOrganization.organization", "employmentContracts"]
        });
    ...
    async getAllContactPersons(organizationId: number): Promise<any[]> {
        ...
        return getRepository(ContactPerson)
        .createQueryBuilder("contactPerson")
        .innerJoinAndSelect("contactPerson.contactPersonOrganizations", "contactPersonOrganization", "contactPersonOrganization.organization.id = " + organizationId)
        .leftJoinAndSelect("contactPersonOrganization.employee", "employee", "employee.contactPersonOrganization.id = contactPersonOrganization.id")
        .leftJoinAndSelect("employee.employmentContracts", "employmentContract", "employmentContract.employee.id = employee.id")
        .getMany();
    }

    async updateAndReturn(id: number, subContractor: SubContractorDTO): Promise<any|undefined> {
        await this.manager.transaction(async transactionalEntityManager => {    
            ...
            let contactPersonObj = await transactionalEntityManager.findOne(ContactPerson, subContractorObj.contactPersonOrganization.contactPerson.id);
            if(!contactPersonObj) {
                throw Error("Sub Contractor not found");
            }
            ...
            let subContractorContract = await transactionalEntityManager.getRepository(EmploymentContract)
            .createQueryBuilder("employmentContract").where(qb => {
                return "start_date = " + qb.subQuery().select("Max(start_date)").from("employment_contracts", 'e').where("employee_id = " + subContractorObj.id).getSql();
            }).andWhere("employee_id = " + subContractorObj.id).getOne();
    ...
    async findOneCustom(id: number): Promise<any|undefined> {
        return this.findOne(id, { relations: ["contactPersonOrganization", "contactPersonOrganization.contactPerson", "contactPersonOrganization.organization", "employmentContracts"] });
    }
