...
export class TimeOffPolicyRepository extends Repository<TimeOffPolicy> {
  async createAndSave(timeOffPolicy: TimeOffPolicyDTO): Promise<any> {
    ...
    id = await this.manager.transaction(async (transactionalEntityManager) => {
      ...
      timeOffPolicyObj = await transactionalEntityManager.save(
        timeOffPolicyObj
      );
      id = timeOffPolicyObj.id;
      let timeOffTypeList = await transactionalEntityManager.findByIds(
        TimeOffType,
        timeOffPolicy.timeOffPolicyTimeOffTypes.map((x) => x.timeOffTypeId)
      );
      console.log('timeOffTypeList.length: ', timeOffTypeList.length);

      let timeOffPolicyTimeOffTypes =
        timeOffPolicy.timeOffPolicyTimeOffTypes.map(
          (timeOffPolicyTimeOffType) => {
            let timeOffPolicyTimeOffTypeObj = new TimeOffPolicyTimeOffType();
            let timeOffType = timeOffTypeList.filter(
              (x) => x.id == timeOffPolicyTimeOffType.timeOffTypeId
            );
            ...
            timeOffPolicyTimeOffTypeObj.earnHours =
              timeOffPolicyTimeOffType.earnHours;
            timeOffPolicyTimeOffTypeObj.earnEvery =
              timeOffPolicyTimeOffType.earnEvery;
            timeOffPolicyTimeOffTypeObj.resetHours =
              timeOffPolicyTimeOffType.resetHours;
            timeOffPolicyTimeOffTypeObj.resetEvery =
              timeOffPolicyTimeOffType.resetEvery;
            timeOffPolicyTimeOffTypeObj.threshold =
              timeOffPolicyTimeOffType.threshold;
            return timeOffPolicyTimeOffTypeObj;
          }
        );

      timeOffPolicyTimeOffTypes = await transactionalEntityManager.save(
        timeOffPolicyTimeOffTypes
      );
  ...
  async getAllActive(): Promise<any[]> {
    return this.find({
      relations: [
        'timeOffPolicyTimeOffTypes',
        'timeOffPolicyTimeOffTypes.timeOffType',
      ],
    });
  }
  ...
  async updateAndReturn(
    id: number,
    timeOffPolicy: TimeOffPolicyDTO
    ...
    await this.manager.transaction(async (transactionalEntityManager) => {
      ...
      let timeOffPolicyTimeOffTypesPromise =
        timeOffPolicy.timeOffPolicyTimeOffTypes.map(
          async (timeOffPolicyTimeOffType) => {
            let timeOffPolicyTimeOffTypeObj:
              | TimeOffPolicyTimeOffType
              | undefined;
            timeOffPolicyTimeOffTypeObj =
            ...
            console.log(
              'timeOffPolicyTimeOffTypeObj - found or not: ',
              timeOffPolicyTimeOffTypeObj
            );
      ...
      let timeOffPolicyTimeOffTypes = await Promise.all(
        timeOffPolicyTimeOffTypesPromise
      );
