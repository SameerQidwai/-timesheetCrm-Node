...
export class EmploymentContractRepository extends Repository<EmploymentContract> {
    ...
    async createAndSave(employmentContract: EmploymentContractDTO): Promise<any> {
        let employee = await this.manager.findOne(Employee,employmentContract.employeeId);
        if(!employee) {
    ...
    async updateAndReturn(id: number, employmentContract: EmploymentContractDTO): Promise<any|undefined> {
        ...
        // check any overlapping contract
        let { count } = await this.createQueryBuilder("employmentContract")
        .select("Count(*)", "count").where("(employee_id = " + employee.id + " AND id <> " + id + ")").andWhere("(end_date is NULL OR FROM_UNIXTIME(" + employmentContract.startDate + "/1000) <= end_date) AND (" + (employmentContract.endDate || "NULL" ) + " is NULL OR start_date <= FROM_UNIXTIME(" + employmentContract.endDate+"/1000))" ).getRawOne();
        ...
        employmentContractObj.remunerationAmount = employmentContract.remunerationAmount; 
        employmentContractObj.remunerationAmountPer = employmentContract.remunerationAmountPer; 
