import { OpportunityDTO, OpportunityResourceAllocationDTO, OpportunityResourceDTO, ProjectDTO, ProjectResourceDTO } from "../dto";
...
export class ProjectRepository extends Repository<Opportunity> {
    ...
    async createAndSave(project: ProjectDTO): Promise<any> {
        ...
        id = await this.manager.transaction(async transactionalEntityManager => {
            ...
            if (project.organizationId) {
                organization = await this.manager.findOne(Organization, project.organizationId);
                if (!organization) {
            ...
            if (project.contactPersonId) {
                contactPerson = await this.manager.findOne(ContactPerson, project.contactPersonId);
                if (!contactPerson) {
    ...
    async updateAndReturn(id: number, project: ProjectDTO): Promise<any|undefined> {
    ...
    async addResource(projectId: number, projectResourceDTO: ProjectResourceDTO) {
       let id = await this.manager.transaction(async transactionalEntityManager => {
            if (!projectId) {
            ...
            resource.panelSkillStandardLevelId = projectResourceDTO.panelSkillStandardLevelId;
            ...
            resourceAllocation.isMarkedAsSelected = projectResourceDTO.isMarkedAsSelected;
            if(projectResourceDTO.contactPersonId) {
                resourceAllocation.contactPersonId = projectResourceDTO.contactPersonId;
            }
            resourceAllocation.opportunityResourceId = resource.id;
            resourceAllocation = await transactionalEntityManager.save(resourceAllocation);
            return resource.id;
        });
    ...
    async updateResource(projectId: number, id: number, projectResourceDTO: ProjectResourceDTO) {
        await this.manager.transaction(async transactionalEntityManager => {
            if (!projectId) {
                throw new Error("Project not found!");
            }
    ...
    async findOneCustomResource(projectId: number, id: number): Promise<any|undefined> {
        if (!projectId) {
            throw new Error("Project not found!");
        }
        let project = await this.findOne(projectId, {
    ...
    // not being used. not working either
    async deleteCustomResource(projectId: number, id: number): Promise<any|undefined> {
        if (!projectId) {
            throw new Error("Project not found!");
        }
        let project = await this.findOne(projectId, {
        ...
        project.opportunityResources = project.opportunityResources.filter(x => x.id !== id);
