...
export class SubContractorContractRepository extends Repository<EmploymentContract> {
    ...
    async createAndSave(contract: ContractDTO): Promise<any> {
        let employee = await this.manager.findOne(Employee,contract.subContractorId);
        if(!employee) {
        ...
        .andWhere("(end_date is NULL OR FROM_UNIXTIME(" + contract.startDate + "/1000) <= end_date) AND (" + (contract.endDate || "NULL" ) + " is NULL OR start_date <= FROM_UNIXTIME(" + contract.endDate+"/1000))" ).getRawOne();
    ...
    async updateAndReturn(id: number, contract: ContractDTO): Promise<any|undefined> {
        ...
        // check any overlapping contract
        let { count } = await this.createQueryBuilder("employmentContract")
        .select("Count(*)", "count").where("(employee_id = " + employee.id + " AND id <> " + id + ")").andWhere("(end_date is NULL OR FROM_UNIXTIME(" + contract.startDate + "/1000) <= end_date) AND (" + (contract.endDate || "NULL" ) + " is NULL OR start_date <= FROM_UNIXTIME(" + contract.endDate+"/1000))" ).getRawOne();
