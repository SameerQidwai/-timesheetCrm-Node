...
export class CalendarHolidayRepository extends Repository<CalendarHoliday> {
  async createAndSave(calendarHoliday: CalendarHolidayDTO): Promise<any> {
    let calendar = await this.manager.findOne(
      Calendar,
      calendarHoliday.calendarId
    );
    let holidayType = await this.manager.findOne(
      HolidayType,
      calendarHoliday.holidayTypeId
    );
    if (!calendar) {
  ...
  async updateAndReturn(
    id: number,
    calendarHoliday: CalendarHolidayDTO
  ...
  async getAllActiveKeyValue(): Promise<any[]> {
    let dbHolidays = await this.find({ relations: ['holidayType'] });
    let holidays: any = {};

    dbHolidays.forEach((holiday) => {
      holidays[moment(holiday.date).format('YYYY-MM-DD')] =
