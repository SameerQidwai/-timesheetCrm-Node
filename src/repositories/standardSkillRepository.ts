...
export class StandardSkillRepository extends Repository<StandardSkill> {
  async createAndSave(standardSkill: StandardSkillDTO): Promise<any> {
    ...
    id = await this.manager.transaction(async (transactionalEntityManager) => {
      ...
      standardSkillObj = await transactionalEntityManager.save(
        standardSkillObj
      );
      id = standardSkillObj.id;
      let standardLevelList = await transactionalEntityManager.findByIds(
        StandardLevel,
        standardSkill.standardSkillStandardLevels.map((x) => x.standardLevelId)
      );
      console.log('standardLevelList.length: ', standardLevelList.length);

      let standardSkillStandardLevels =
        standardSkill.standardSkillStandardLevels.map(
          (standardSkillStandardLevel) => {
            let standardSkillStandardLevelObj =
              new StandardSkillStandardLevel();
            let standardLevel = standardLevelList.filter(
              (x) => x.id == standardSkillStandardLevel.standardLevelId
            );
            ...
            standardSkillStandardLevelObj.priority =
              standardSkillStandardLevel.priority;
            return standardSkillStandardLevelObj;
          }
        );

      standardSkillStandardLevels = await transactionalEntityManager.save(
  ...
  async getAllActive(): Promise<any[]> {
    return this.find({
      relations: [
        'standardSkillStandardLevels',
        'standardSkillStandardLevels.standardLevel',
      ],
    });
  }
  ...
  async updateAndReturn(
    id: number,
    standardSkill: StandardSkillDTO
    ...
    await this.manager.transaction(async (transactionalEntityManager) => {
      ...
      let standardSkillStandardLevelsPromise =
        standardSkill.standardSkillStandardLevels.map(
          async (standardSkillStandardLevel) => {
            let standardSkillStandardLevelObj:
              | StandardSkillStandardLevel
              | undefined;
            standardSkillStandardLevelObj =
            ...
            console.log(
              'standardSkillStandardLevelObj - found or not: ',
              standardSkillStandardLevelObj
            );
      ...
      let standardSkillStandardLevels = await Promise.all(
        standardSkillStandardLevelsPromise
      );
      standardSkillObj['standardSkillStandardLevels'] =
        standardSkillStandardLevels;
  ...
  async helplerGetLevelsBySkill(skillId: number): Promise<any[]> {
    ...
    if (isNaN(skillId)) {
      throw new Error('Undefined Skills ID');
    }
    let skill = await this.find({
      relations: [
        'standardSkillStandardLevels',
        'standardSkillStandardLevels.standardLevel',
      ],
